...
async function fetchCommitActivityWithRetry(owner, repo, maxRetries = 3, delay = 2000) {
  let retries = 0;
  while (retries < maxRetries) {
    ...
    if (response.status === 200 && Array.isArray(response.data)) {
      ...
      // Data is being generated by GitHub
      await new Promise(res => setTimeout(res, delay));
      retries++;
    } else {
      throw new Error(`Failed to fetch commit activity. Status: ${response.status}`);
    }
  }
  throw new Error('GitHub commit activity not ready after retries');
}
